package protonvpn

import (
	"strings"
)

// Proton OpenVPN CA and tls-crypt key are public material embedded in Proton
// OpenVPN profiles. Keeping them here lets Tundler generate headless configs
// from automatically refreshed server metadata.
const protonCA = ""

const protonTLSCrypt = ""

func buildOpenVPNConfig(server *protonServer, credentialsFile string) string {
	proto := getProtocol()
	port := getPort(proto)

	var lines []string
	lines = append(lines,
		"client",
		"dev tun",
		"proto "+proto,
	)

	remotes := server.IPs
	if len(remotes) == 0 {
		remotes = []string{server.Hostname}
	}
	for _, remote := range remotes {
		if strings.TrimSpace(remote) != "" {
			lines = append(lines, "remote "+remote+" "+port)
		}
	}

	lines = append(lines,
		"remote-random",
		"resolv-retry infinite",
		"nobind",
		"cipher AES-256-GCM",
		"data-ciphers AES-256-GCM:AES-128-GCM",
		"data-ciphers-fallback AES-256-GCM",
		"auth SHA512",
		"verb 3",
		"setenv CLIENT_CERT 0",
		"tun-mtu 1500",
		"tun-mtu-extra 32",
		"mssfix 1450",
		"persist-key",
		"persist-tun",
		"reneg-sec 0",
		"remote-cert-tls server",
		"auth-user-pass "+credentialsFile,
		"pull",
		"auth-nocache",
		"auth-retry nointeract",
		"pull-filter ignore \"auth-token\"",
		"pull-filter ignore \"tun-ipv6\"",
		"pull-filter ignore \"route-ipv6\"",
		"pull-filter ignore \"ifconfig-ipv6\"",
	)
	if proto == "udp" {
		lines = append(lines, "explicit-exit-notify", "fast-io")
	}

	lines = append(lines,
		"<ca>",
		"-----BEGIN CERTIFICATE-----",
		protonCA,
		"-----END CERTIFICATE-----",
		"</ca>",
		"<tls-crypt>",
		"-----BEGIN OpenVPN Static key V1-----",
		protonTLSCrypt,
		"-----END OpenVPN Static key V1-----",
		"</tls-crypt>",
		"",
	)

	return strings.Join(lines, "\n")
}
