...
printenv | grep -E '^(POD_|EXPRESSVPN_|FASTVPN_|IPVANISH_|MULLVAD_|NORDVPN_|OVPN_|PIA_|PRIVATEINTERNETACCESS_|PROTON_|PUREVPN_|SURFSHARK_|TUNNELBEAR_|VEEPN_|WINDSCRIBE_|TUNDLER_)|^(BOOT_LOGIN_JITTER_SECONDS|EXCLUDED_LOCATIONS|MIN_ROTATION_SECONDS|MAX_ROTATION_SECONDS|TUNNEL_WATCHDOG_INTERVAL_SECONDS|WEDGE_GUARD_THRESHOLD_SECONDS|ROTATION_RETRY_MAX|ROTATION_ATTEMPT_TIMEOUT_SECONDS|RECYCLE_AFTER_SECONDS|RECYCLE_AFTER_ROTATIONS)=' | sort > "$ENV_FILE"
