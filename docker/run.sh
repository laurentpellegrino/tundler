...
docker run -it -d \
  ...
  -p 4242:4242 \
  -p 8484:8484 \
  -e EXPRESSVPN_ACTIVATION_CODE \
  -e MULLVAD_ACCOUNT_NUMBER \
  -e NORDVPN_TOKEN \
  -e PRIVATEINTERNETACCESS_PASSWORD \
  -e PRIVATEINTERNETACCESS_USERNAME \
  -e SURFSHARK_OPENVPN_USERNAME \
  -e SURFSHARK_OPENVPN_PASSWORD \
