...
func TestStartProvider_InitialSyncFromPrePopulatedSlices(t *testing.T) {
	cs := fake.NewSimpleClientset(
		newSlice("tundler-tunnel-expressvpn-abc1", "tundler-tunnel-expressvpn",
			readyEndpoint("tundler-tunnel-expressvpn-0", "10.0.0.1"),
			readyEndpoint("tundler-tunnel-expressvpn-1", "10.0.0.2"),
			notReadyEndpoint("tundler-tunnel-expressvpn-2", "10.0.0.3"),
		),
	)
	...
	defer cancel()
	stop, done := w.startProvider(ctx, "tundler-tunnel-expressvpn", "expressvpn")
	defer stop()
	...
	svc0, ok0 := fc.podToService["tundler-tunnel-expressvpn-0"]
	_, okNotReady := fc.podToService["tundler-tunnel-expressvpn-2"]
	fc.mu.RUnlock()
	...
	if !ok0 || svc0 != "tundler-tunnel-expressvpn" {
		t.Errorf("podToService[tundler-tunnel-expressvpn-0]=(%q,%v), want (tundler-tunnel-expressvpn,true)", svc0, ok0)
	}
	if okNotReady {
...
func TestStartProvider_EmptySlicesStillSignalsReady(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	stop, done := w.startProvider(ctx, "tundler-tunnel-surfshark", "surfshark")
	defer stop()
	waitForInitialSync(t, done)
...
func TestApplyProviderReconcile_FullReplaceSemantics(t *testing.T) {
	...
	if _, ok := fc.podToService["tundler-tunnel-expressvpn-0"]; !ok {
		t.Error("podToService[tundler-tunnel-expressvpn-0] missing after first reconcile")
	}
	...
	fc.applyProviderReconcile("expressvpn", "tundler-tunnel-expressvpn",
		...
		[]string{"tundler-tunnel-expressvpn-1", "tundler-tunnel-expressvpn-2"},
	)
	if got := fc.healthy["expressvpn"]; got != 2 {
		t.Errorf("after second reconcile: healthy=%d, want 2", got)
	}
	if _, ok := fc.podToService["tundler-tunnel-expressvpn-0"]; ok {
		t.Error("tundler-tunnel-expressvpn-0 should have been dropped on second reconcile")
	}
	if _, ok := fc.podToService["tundler-tunnel-expressvpn-2"]; !ok {
		t.Error("tundler-tunnel-expressvpn-2 should have been added on second reconcile")
	}
...
func TestApplyProviderReconcile_OneProviderDoesNotTouchAnother(t *testing.T) {
	...
	fc.applyProviderReconcile("nordvpn", "tundler-tunnel-nordvpn",
		[]string{"10.1.0.1", "10.1.0.2"},
		[]string{"tundler-tunnel-nordvpn-0", "tundler-tunnel-nordvpn-1"},
	)
...
func TestApplyProviderReconcile_EmptyDropsAllPodsForProvider(t *testing.T) {
	...
	fc.applyProviderReconcile("expressvpn", "tundler-tunnel-expressvpn", nil, nil)
